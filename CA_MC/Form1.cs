...
namespace CA_MC
{
    public partial class Form1 : Form
    {
        ...
        int neighbourhoodRadius, MCiterations;
        int time = 2;
        double kT = 1.0;
        Color[] grainColors;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            MCiterations = Convert.ToInt32(numericUpDown6.Value);
            kT = Convert.ToInt32(numericUpDown7.Value)/10.0;

            Random rnd = new Random();

            MCtab = new int[width, height,1];
            visitedTab = new bool[width, height];
            for (int i = 0; i < width; i++)
                for (int j = 0; j < height; j++)
                {
                    MCtab[i, j, 0] = tab[i, j, 1];
                    visitedTab[i, j] = false;
                }
            ...
            for (int mc=0; mc<MCiterations; mc++)
            {
                ...
                int visited = 0;
                while(visited != grainQuantity)
                {
                    int ii = rnd.Next(width);
                    int jj = rnd.Next(height);

                    while (visitedTab[ii,jj] != false)
                    {
                        ii = rnd.Next(width);
                        jj = rnd.Next(height);
                    }

                    int[] neighbours = getNeighbours(MCtab, ii, jj, neighbourhood);
                    int currentE = calculateEnergy(MCtab[ii, jj, 0], neighbours);

                    int potentialV = neighbours[rnd.Next(neighbours.Length)];
                    while (potentialV == 0) potentialV = neighbours[rnd.Next(neighbours.Length)];

                    int potentialE = calculateEnergy(potentialV, neighbours);

                    int deltaE = potentialE - currentE;

                    if(deltaE <=0)
                    {
                        MCtab[ii, jj, 0] = potentialV;
                    }
                    else if(deltaE>0)
                    {
                        double exp=Math.Exp(-deltaE/kT);
                        double rand = rnd.Next(101)*1.0;
                        if (rand<exp)
                        {
                            MCtab[ii, jj, 0] = potentialV;
                        }
                ...
                if (checkBox2.Checked == false)
                {
                    for (int i = 0; i < width; i++)
                        for (int j = 0; j < height; j++)
                        {
                            g.FillRectangle(new SolidBrush(grainColors[MCtab[i, j, 0] - 1]), i * grainPictureSize, j * grainPictureSize, grainPictureSize, grainPictureSize);
                        }
        ...
        private int calculateEnergy(int cellValue, int[] neighbours)
        {
            int energy = 0;

            for (int i=0; i<neighbours.Length; i++)
            {
                if (neighbours[i] != cellValue) energy++;
            }
